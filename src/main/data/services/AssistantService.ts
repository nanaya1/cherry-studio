...
function rowToAssistant(
  ...
  return {
    ...
    // MEA 定制：内置助手身份归一化（schema 是 'assistant' | null，行数据可能 undefined）
...
export class AssistantDataService {
  ...
  delete(
    ...
    const { deleted, deletedTopicIds, projectedTopicIds } = application.get('DbService').withWriteTx((tx) => {
      ...
      const [existing] = tx
        .select({ id: assistantTable.id, builtinRole: assistantTable.builtinRole })
        .from(assistantTable)
        .where(predicate)
        .limit(1)
        .all()
      if (!existing) throw DataApiErrorFactory.notFound('Assistant', id)
      // MEA 定制：内置默认助手（builtinRole === 'assistant'）禁止删除
      if (existing.builtinRole === 'assistant') {
